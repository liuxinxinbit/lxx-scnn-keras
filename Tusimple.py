...
class Tusimple():
    ...
    def __init__(self, path='../Tusimple', image_set='test'):
    ...
    def BatchGenerator(self, batch_size=6, image_size=(352, 640, 3), labels=5):
        index_set = range(len(self.img_list))
        while True:
            index = randint(0, self.num_train-batch_size)
            images = []
            truths_set = []
            exist_list_set = []
            for j in range(batch_size):
                ...
                truth_mask = np.expand_dims(truth_mask, -1)
                truth_mask = tf.image.resize(
                    truth_mask, [352, 640], method=ResizeMethod.NEAREST_NEIGHBOR).numpy()

                image, truth_mask = data_augment(image, truth_mask[:, :, 0])
                images.append(image)
